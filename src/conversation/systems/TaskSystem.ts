...
export class TaskSystem implements System {
  ...
  async execute(context: HookContext): Promise<boolean> {
    ...
    // 3.5. Auto-inject explainImage task when the message contains image segments.
    // This makes image handling explicit and task-driven: ExplainImageTaskExecutor is always
    // responsible for image description, removing the need for implicit extraction downstream.
    // Pass segments in parameters so the executor receives them directly (avoids context/message
    // propagation issues where segments might not be available when the executor runs).
    // Only add if not already generated by LLM analysis above.
    const hasImageSegments = context.message.segments?.some((seg) => seg.type === 'image') ?? false;
    if (hasImageSegments && !additionalTasks.some((t) => t.type === 'explainImage')) {

...
import type { ConversationContext, ProactiveReplyInjectContext } from '@/context/types';
...
export class AIService {
  ...
  /**
   * Analyze a text message for tasks without a full HookContext.
   * Used by flows that do not go through the standard message pipeline, e.g. proactive conversation.
   * Returns task array (excluding the reply task which is always generated by the system).
   *
   * @param userMessage - The message text to analyze (e.g. last group message, topic from Ollama)
   * @param groupId - Optional group ID for context
   * @param sessionId - Optional session ID (unused by analysis itself but kept for future extension)
   */
  async analyzeProactiveTasks(
    userMessage: string,
    groupId?: number,
    sessionId?: string,
  ): Promise<Task[]> {
    const context: ConversationContext = {
      ...
      groupId,
      messageType: 'group',
      metadata: new Map(sessionId ? [['sessionId', sessionId]] : []),
    };

    try {
      const result = await this.taskAnalysisService.analyze(context);
      return result.tasks;
    } catch (error) {
      logger.warn('[AIService] analyzeProactiveTasks failed:', error instanceof Error ? error.message : String(error));
      return [];
    }
  }

  /**
   * Check whether a message text contains trigger keywords for any registered non-reply task type.
   * Used by non-pipeline flows (e.g. proactive conversation) to gate the expensive LLM task analysis call.
   *
   * @param text - Message text to scan
   * @returns true when at least one non-reply task type has a matching trigger keyword
   */
  hasProactiveTaskTriggers(text: string): boolean {
    const messageLower = text.toLowerCase();
    for (const taskType of this.taskManager.getAllTaskTypes()) {
      if (taskType.name.toLowerCase() === 'reply') continue;
      if (taskType.triggerKeywords?.some((kw) => messageLower.includes(kw.toLowerCase()))) {
        return true;
      }
    }
    return false;
  ...
  /**
   * Execute a single search query and return formatted results text.
   * Used by non-pipeline flows (e.g. proactive conversation task execution) that cannot go through
   * the full TaskSystem but still want to run a search task produced by analyzeProactiveTasks().
   *
   * @param query - Search query string
   * @returns Formatted search results text, or empty string when search is unavailable or fails
   */
  async searchForProactive(query: string): Promise<string> {
    if (!this.searchService?.isEnabled()) {
      logger.info('[AIService] searchForProactive: SearchService not enabled, skipping');
      return '';
    }
    try {
      ...
      logger.warn('[AIService] searchForProactive failed:', error instanceof Error ? error.message : String(error));
      return '';
    }
  }

...
@Tool({
  ...
  description:
    '从已收录的微信公众号文章库中语义搜索相关内容。' +
    '文章库包含大量高质量的资讯、行业分析和深度报道，覆盖科技、财经、时事等领域。' +
    '适用于：1) 查找近期热点事件和新闻资讯；2) 获取某个话题的高质量分析和观点；3) 用户明确要求查找微信/公众号相关内容。' +
  ...
  whenToUse:
    ...
    '3) 与 wechat_search（关键词匹配）不同，本工具使用语义向量搜索，能理解同义词和相近表述，适合模糊话题检索。',
})
@injectable()

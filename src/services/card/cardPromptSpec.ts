...
export const CARD_TYPE_SPECS: CardTypeSpecEntry[] = [
  ...
  },
  {
    ...
    optionalNote:
      '用于承载原文中的自然语言段落、过渡句、总结语等非结构化内容。**禁止连续出现多个 `paragraph` 卡片**——若相邻内容都是自然语言段落，必须合并为一个 `paragraph`，在 `content` 内用多个 `<p>` 标签分段，或用 `<span>`、`<br>` 等 HTML 元素组合。content 内使用 HTML（`<p>`、`<strong>`、`<em>`、`<code>` 等）。',
  },
];

...
describe('VideoDownloadService', () => {
  ...
  it('falls back to HTTP when yt-dlp is unavailable', async () => {
    ...
    vi.spyOn(fsPromises, 'mkdir').mockResolvedValue(undefined);
    const downloadToBase64 = vi.spyOn(ResourceDownloader, 'downloadToBase64').mockResolvedValue(
      Buffer.from('http-bytes').toString('base64'),
    );
  ...
  it('uses the HTTP path for generic URLs', async () => {
    ...
    vi.spyOn(fsPromises, 'mkdir').mockResolvedValue(undefined);
    const downloadToBase64 = vi.spyOn(ResourceDownloader, 'downloadToBase64').mockResolvedValue(
      Buffer.from('generic-http').toString('base64'),
    );

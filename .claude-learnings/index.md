...
| avatar | [avatar.md](avatar.md) | Preview WS 协议（frame/status 消息结构）、LAN 配置规则（0.0.0.0:8002 + location.hostname）、Bun.serve idleTimeout=255、latestStatus 缓存连接时立即下发；BotState 5 态状态机、转换动画表、idle 随机待机动画（setTimeout 链）、StateNodeOutput 类型独立于 compiler |

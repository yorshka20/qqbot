...
describe('PromptInjectionRegistry.gatherByLayer', () => {
  ...
  beforeEach(() => { registry = new PromptInjectionRegistry(); });

  it('groups producers by their declared layer', async () => {
    const baseline: PromptInjectionProducer = { name: 'b', layer: 'baseline', produce: () => ({ producerName: 'b', fragment: 'B' }) };
    const scene: PromptInjectionProducer = { name: 's', layer: 'scene', produce: () => ({ producerName: 's', fragment: 'S' }) };
    const runtime: PromptInjectionProducer = { name: 'r', layer: 'runtime', produce: () => ({ producerName: 'r', fragment: 'R' }) };
    const tool: PromptInjectionProducer = { name: 't', layer: 'tool', produce: () => ({ producerName: 't', fragment: 'T' }) };
  ...
  it('sorts within a layer by ascending priority (default 100)', async () => {
    const a: PromptInjectionProducer = { name: 'a', layer: 'runtime', priority: 100, produce: () => ({ producerName: 'a', priority: 100, fragment: 'a' }) };
    const b: PromptInjectionProducer = { name: 'b', layer: 'runtime', priority: 10, produce: () => ({ producerName: 'b', priority: 10, fragment: 'b' }) };
    const c: PromptInjectionProducer = { name: 'c', layer: 'runtime', produce: () => ({ producerName: 'c', fragment: 'c' }) }; // default 100
  ...
  it('throwing producer does not break other layers', async () => {
    ...
    const thrower: PromptInjectionProducer = { name: 'thrower', layer: 'baseline', produce: () => { throw new Error('boom'); } };
    const good: PromptInjectionProducer = { name: 'good', layer: 'scene', produce: () => ({ producerName: 'good', fragment: 'G' }) };

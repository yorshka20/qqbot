...
describe('AuditEventStore', () => {
  it('records and returns events oldest-first', () => {
    const store = new AuditEventStore({ maxAgeMs: 60_000 });
  ...
  it('retention is time-based only — no item cap', () => {
    const store = new AuditEventStore({ maxAgeMs: 60_000 });
    for (let i = 0; i < 50; i++) {
      store.record('g', { ts: T0 + i, kind: 'tool', summary: `t${i}` });
    }
    expect(store.getRecent('g', T0 + 100).length).toBe(50);
  });

  it('prunes events older than maxAgeMs', () => {
    const store = new AuditEventStore({ maxAgeMs: 5_000 });

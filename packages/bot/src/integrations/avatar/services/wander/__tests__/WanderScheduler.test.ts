...
import { DEFAULT_PERSONA_CONFIG, type PersonaConfig } from '@/persona/types';
...
const wanderConfig = (overrides: Partial<PersonaConfig['wander']> = {}): PersonaConfig['wander'] => ({
  ...DEFAULT_PERSONA_CONFIG.wander,
  ...overrides,
});
...
describe('pickIntent — step shape per kind', () => {
  test('glance with head-still target emits setGaze → wait → clear', () => {
    const config = wanderConfig({ intents: { ...DEFAULT_PERSONA_CONFIG.wander.intents, glance: 1 } });
  ...
  test('look_around produces head-only sweep (setHead → wait → setHead → wait → release)', () => {
    const config = wanderConfig({
      intents: { ...DEFAULT_PERSONA_CONFIG.wander.intents, glance: 0, look_around: 1 },
    });

...
export class MemoryFactMetaService {
  ...
  /**
   * Every fact row, for the memory browser. Callers group and sort.
   */
  listAllFacts(): FactMeta[] {
    return this.db.query('SELECT * FROM memory_fact_meta').all() as FactMeta[];
  }
  ...
  lookupLatestGroupNames(groupIds: string[]): Map<string, string> {
    ...
    try {
      ...
      logger.warn('[MemoryFactMetaService] group name lookup failed:', err);
    }
    return names;
  }
  ...
  lookupLatestNicknames(subjects: Array<{ groupId: string; userId: string }>): Map<string, string> {
    ...
    try {
      ...
      logger.warn('[MemoryFactMetaService] nickname lookup failed:', err);
    }
    return names;
  }
}

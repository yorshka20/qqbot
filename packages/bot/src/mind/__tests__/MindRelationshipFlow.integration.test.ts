...
import { RelationshipUpdater } from '../relationships/RelationshipUpdater';
import { DEFAULT_MIND_CONFIG, mergeMindConfig } from '../types';
...
function mindSnapshot(enabled = true) {
  const config = mergeMindConfig({ enabled });
  return {
    enabled,

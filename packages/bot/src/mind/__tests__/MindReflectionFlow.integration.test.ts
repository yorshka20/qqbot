...
//   3. After successful write, MindService.setCurrentTone is called.
...
import { DEFAULT_MIND_CONFIG } from '../types';
...
beforeEach(async () => {
  ...
  mind = new MindService({ ...DEFAULT_MIND_CONFIG, enabled: true }, new InternalEventBus());
...
describe('MindReflectionFlow integration', () => {
  it('fake LLM cycle updates store + MindService tone; next buildPromptPatchAsync observes tonePromptFragment', async () => {
    const personaId = DEFAULT_MIND_CONFIG.personaId;
    ...
    // MindService tone should have been updated to 'playful'

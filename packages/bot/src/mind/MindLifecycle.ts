 ...
 *
 * Bootstrap calls `startMindSubsystem(...)` once; all of the above is
...
export interface MindLifecycleHandles {
 ...
 *
 * Behaviour:
 *  - `mindService.isEnabled() === false` → returns immediately, no-op
 ...
 */
export function startMindSubsystem(
  ...
  if (avatarService) {
    attachAvatar(mindService, modulationProvider, avatarService);
  }

  mindService.start();
  const cfg = mindService.getConfig();
  logger.info(
...
function attachAvatar(mind: MindService, modulationProvider: MindModulationAdapter, avatar: AvatarService): void {

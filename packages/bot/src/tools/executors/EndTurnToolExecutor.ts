...
@Tool({
  name: 'end_turn',
  description:
    '结束本轮回复，不再发送任何内容。当你要说的内容已经全部发出（例如已用 send_card 发出卡片、或已用 send_message 说完），且没有需要补充的话时调用。调用后本轮立即结束。注意：若你仍有内容想说，直接输出文本即可（输出的文本一定会被发送），不要调用本工具。',
  executor: 'end_turn',
  visibility: {
  ...
  parameters: {},
  whenToUse:
    '仅当本轮不需要再发送任何内容时调用（通常发生在 send_card 或 send_message 已把该说的都发出去之后）。它是"我说完了"的明确信号；与输出文本二选一——输出文本表示"这是我要发送的回复"，调用 end_turn 表示"没有更多要发送的了"。',
  examples: [

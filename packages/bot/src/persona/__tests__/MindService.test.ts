    ...
    const mind = new PersonaService(
      {
        ...
        enabled: true,
        promptPatch: { ...DEFAULT_PERSONA_CONFIG.promptPatch, enabled: false },
      },
      bus,
    );
...
describe('mergePersonaConfig', () => {
  test('undefined input returns defaults (disabled)', () => {
    const c = mergePersonaConfig(undefined);
  ...
  test('partial input overlays onto defaults', () => {
    const c = mergePersonaConfig({ enabled: true, tickMs: 500 });
  ...
  test('nested ode overlays shallow-merge', () => {
    const c = mergePersonaConfig({ ode: { tauAttentionMs: 60_000 } });
    expect(c.ode.tauAttentionMs).toBe(60_000);
    // Other ode fields fall back to defaults
    expect(c.ode.attentionSpikePerMessage).toBe(DEFAULT_PERSONA_CONFIG.ode.attentionSpikePerMessage);
  });

  test('invalid types fall back to defaults', () => {
    const c = mergePersonaConfig({ tickMs: 'not-a-number' as unknown as number });
    expect(c.tickMs).toBe(DEFAULT_PERSONA_CONFIG.tickMs);
  });

  test('promptPatch section defaults to enabled + default thresholds', () => {
    const c = mergePersonaConfig({});
    expect(c.promptPatch.enabled).toBe(true);
    expect(c.promptPatch.fatigueMildMin).toBe(DEFAULT_PERSONA_CONFIG.promptPatch.fatigueMildMin);
  });

  test('promptPatch partial override merges over defaults', () => {
    const c = mergePersonaConfig({ promptPatch: { enabled: false } });
    expect(c.promptPatch.enabled).toBe(false);
    expect(c.promptPatch.fatigueSevereMin).toBe(DEFAULT_PERSONA_CONFIG.promptPatch.fatigueSevereMin);
  });
});

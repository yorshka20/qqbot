...
/**
 * Specialised slerp from the identity quaternion (0,0,0,1) to (bx,by,bz,bw).
 * Equivalent to `slerpQuat(0, 0, 0, 1, bx, by, bz, bw, t)` but skips the
 * four zero-weighted multiplies in the general form. Used by the clip
 * path as the fallback when no continuous layer provides a pose anchor
 * (Cubism models, tests without the continuous stack, bones outside the
 * idle loop clip's coverage).
 */
export function slerpWithIdentity(
  bx: number,
  by: number,
  bz: number,
  bw: number,

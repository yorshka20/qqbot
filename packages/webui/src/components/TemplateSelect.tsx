...
function TriggerLabel({
  ...
  const roleTone =
    t.role === 'planner' ? 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300' : 'bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300';
  void isUnknown;
  return (
...
function GroupLabel({ icon, text, tone }: { icon: React.ReactNode; text: string; tone: 'violet' | 'sky' | 'amber' }) {
  ...
  return (
    <Select.Label className={`flex items-center gap-1.5 px-2 py-1.5 text-[10px] uppercase tracking-wider font-semibold ${cls}`}>
      {icon}
      {text}
    </Select.Label>
  );

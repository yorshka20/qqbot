...
Ecosystem has `treekill: false` for `test-restart` (and for `qq-bot`) so PM2 does not kill the helper when it restarts the app.
...
If the log contains at least two `started <ISO timestamp>` lines with different timestamps, the restart-from-within scheme works. The same pattern (spawn helper, helper runs `pm2 restart qq-bot --no-treekill`) is used in the `/restart` command.

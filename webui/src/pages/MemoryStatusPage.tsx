/**
 * Memory Status Page - Quality observation dashboard for memory facts.
 * Shows global stats, per-group breakdown, and per-user fact details.
 */
...
function SourceBadge({ source }: { source: string }) {
  ...
  return <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${cls}`}>{source === 'manual' ? 'manual' : 'auto'}</span>
}
    ...
    <div className="flex-1 overflow-auto p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {view.type === 'overview' && globalStats && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <StatCard icon={<Database className="w-5 h-5 text-purple-500" />} label="Total Facts" value={globalStats.totalFacts} color="bg-purple-100 dark:bg-purple-900/30" />
              <StatCard icon={<Zap className="w-5 h-5 text-emerald-500" />} label="Active" value={globalStats.activeFacts} color="bg-emerald-100 dark:bg-emerald-900/30" />
              <StatCard icon={<Eye className="w-5 h-5 text-amber-500" />} label="Stale" value={globalStats.staleFacts} color="bg-amber-100 dark:bg-amber-900/30" />
              <StatCard icon={<Shield className="w-5 h-5 text-blue-500" />} label="Manual" value={globalStats.manualFacts} color="bg-blue-100 dark:bg-blue-900/30" />
              <StatCard icon={<Brain className="w-5 h-5 text-zinc-500" />} label="Auto (LLM)" value={globalStats.autoFacts} color="bg-zinc-100 dark:bg-zinc-700" />
            </div>
        ...
        {view.type === 'group' && groupDetail && (
          <div className="bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 overflow-hidden">
            <div className="px-4 py-3 border-b border-zinc-200 dark:border-zinc-700">
              <h2 className="font-semibold">Users ({groupDetail.users.length}) - {groupDetail.totalFacts} total facts</h2>
            </div>
            <div className="divide-y divide-zinc-100 dark:divide-zinc-700">
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-700">
                  {userFacts.facts.map((f) => (
                    <tr key={f.factHash} className="hover:bg-zinc-50 dark:hover:bg-zinc-750">
                      ...
                      <td className="px-4 py-2"><SourceBadge source={f.source} /></td>
                      ...
                      <td className="px-4 py-2 text-right text-zinc-400">{formatAge(f.ageDays)}</td>
                      <td className="px-4 py-2 text-zinc-400 text-xs">{formatDate(f.firstSeen)}</td>
                      <td className="px-4 py-2 text-zinc-400 text-xs">{formatDate(f.lastReinforced)}</td>

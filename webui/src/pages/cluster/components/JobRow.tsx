              ...
              {detail.tasks.map((t) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className="font-mono text-xs text-zinc-600 dark:text-zinc-300">{t.id.slice(0, 8)}</div>
                    <ClusterStatusBadge status={t.status} />
                    {t.workerTemplate && (
                      <div className="text-xs font-mono text-zinc-500 dark:text-zinc-400">{t.workerTemplate}</div>
                    )}
                  ...
                  {t.error && <div className="mt-1 text-xs text-red-600 dark:text-red-400 truncate">{t.error}</div>}
                </button>
              ))}
            </div>
          )}

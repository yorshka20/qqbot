...
function Field({ label, value }: { label: string; value: React.ReactNode }) {
  ...
  return (
    <div className="flex items-start gap-2 text-xs">
      <span className="shrink-0 w-36 text-zinc-500 dark:text-zinc-400 font-medium">{label}</span>
...
export function WorkerDetailModal({ worker, onClose }: WorkerDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60">
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-2xl flex flex-col w-[min(95vw,60rem)] max-h-[90vh]">
        {/* Header */}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-4">
          {/* Worker fields */}
          <div className="space-y-1.5 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg p-3 border border-zinc-200 dark:border-zinc-700">
            ...
            <Field label="lastReportSummary" value={worker.lastReportSummary} />
            <Field label="lastReportNextSteps" value={worker.lastReportNextSteps} />

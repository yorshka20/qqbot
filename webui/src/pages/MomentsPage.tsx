...
import { Calendar, Clock, Hash, Loader2, Tag } from 'lucide-react'
import { useCallback, useEffect, useMemo, useState } from 'react'
import { Bar, BarChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts'

import { getMomentsStats, listMoments } from '../api'
...
export function MomentsPage() {
  ...
  const groups = useMemo(() => groupByDate(moments), [moments])
  ...
  return (
    <main className="flex-1 min-h-0 overflow-auto p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* Timeline chart */}
        {stats && stats.monthlyCount.length > 0 && (
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Result count */}
          {!loading && (
            <span className="text-xs text-zinc-400 dark:text-zinc-500 ml-auto">{moments.length} 条结果</span>
          )}
        </div>

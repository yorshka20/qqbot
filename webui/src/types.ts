...
export interface TicketFrontmatter {
   ...
   * planner is allowed to spawn. Only meaningful when the selected
   * `template` has `role: 'planner'` — ignored otherwise. Surfaced to the
   * planner via prompt; the hub does not hard-enforce this number.
   */
  maxChildren?: number;
  /**
